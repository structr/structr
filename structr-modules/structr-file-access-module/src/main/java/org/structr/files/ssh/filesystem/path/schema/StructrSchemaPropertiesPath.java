 ...
 *
 */
public class StructrSchemaPropertiesPath extends StructrPath {

	private static final Logger logger = LoggerFactory.getLogger(StructrSchemaPropertiesPath.class.getName());

	private AbstractSchemaNode schemaNode = null;

	public StructrSchemaPropertiesPath(final StructrFilesystem fs, final StructrPath parent, final AbstractSchemaNode schemaNode) {
		super(fs, parent, "properties");
	...
	public Map<String, Object> getAttributes(final String attributes, final LinkOption... options) throws IOException {
		return new StructrNodeAttributes(fs.getSecurityContext(), schemaNode).toMap("attributes");
	}

	@Override
	public <T extends BasicFileAttributes> T getAttributes(Class<T> type, LinkOption... options) throws IOException {
		return (T)new StructrNodeAttributes(fs.getSecurityContext(), schemaNode);
	}
