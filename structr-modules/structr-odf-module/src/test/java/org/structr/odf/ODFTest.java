...
public class ODFTest extends StructrODFModuleTest {
	...
	public void testODS() {
		...
		RestAssured
			...
			.expect()
			.statusCode(200)
			.body("result.type",                  equalTo("ODSExporter"))
			.body("result.documentTemplate.type", equalTo("File"))
			.body("result.documentTemplate.name", equalTo("template"))
			.body("result.documentTemplate.path", equalTo("/template"))
			.body("result.resultDocument.type",   equalTo("File"))
			.body("result.resultDocument.name",   equalTo("test.ods_template"))
			.body("result.resultDocument.path",   equalTo("/test.ods_template"))
			.when()
			.get("/ODFExporter");
	}
}
