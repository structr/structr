 ...
 */
package org.structr.sql.index.predicate;

import org.structr.api.Predicate;
import org.structr.api.graph.PropertyContainer;
import org.structr.sql.SQLNode;
import org.structr.sql.SQLRelationship;
...
public class LabelPredicate<T extends PropertyContainer> implements Predicate<T> {
	...
	public boolean accept(final T entity) {
		...
		if (entity instanceof SQLNode) {

			final SQLNode node = (SQLNode)entity;

			return node.hasLabel(label);
		}

		if (entity instanceof SQLRelationship) {

			final SQLRelationship relationship = (SQLRelationship)entity;
			final SQLNode sourceNode           = (SQLNode)relationship.getStartNode();
			final SQLNode targetNode           = (SQLNode)relationship.getEndNode();
