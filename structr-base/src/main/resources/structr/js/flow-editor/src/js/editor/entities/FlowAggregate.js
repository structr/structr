...
export class FlowAggregate extends FlowNode {
        ...
        return `
            <div class="title {{node.isStartNode ? 'startNode' : ''}}">{{node.title}}</div>
                ...
                <content al-repeat="control in node.controls">
                    ...
                    <column>
                        <div class="control" id="{{control.id}}" style="text-align: right" :width="control.parent.width - 2 * control.margin" :height="control.height" al-control="control"></div>
                    </column>
                </content>
            </div>
        `;
