...
public class FilterExpression extends Expression {
	...
	public List<Parameter> getParameters() {
		...
		return List.of(Parameter.mandatory("list", "list of elements to loop over"), Parameter.mandatory("filterExpression", "expression to evaluate for each element"));
	}

	@Override
	public List<Example> getExamples() {

		return List.of(Example.structrScript("${filter(find('User'), not(data.isAdmin))}", "Remove admin users from a list of users"));
	}

	@Override
	public List<String> getNotes() {

		return List.of("This function is only available in StructrScript because there is a native language feature in JavaScript that does the same (`Array.prototype.filter()`).");
	}

	@Override
	public List<Signature> getSignatures() {

		return List.of(Signature.structrScript("list, filterExpression"));
	}
	...
	public List<Usage> getUsages() {
		...
		return List.of(Usage.structrScript("Usage: ${filter(list, filterExpression)}. Example: ${filter(find('User'), data.isAdmin)}"));
	}
