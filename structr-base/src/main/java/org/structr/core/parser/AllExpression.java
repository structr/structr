...
public class AllExpression extends Expression {
	...
	public AllExpression(final int row, final int column) {

		super("all", row, column);
	}
	...
	public void add(final Expression expression) throws FrameworkException {
		...
		} else if (this.allExpression == null) {

			this.allExpression = expression;

		} else {

			throw new FrameworkException(422, "Invalid all() expression in builtin function: too many parameters.");
		}
	...
	public List<Usage> getUsages() {
		...
		return List.of(Usage.structrScript("Usage: ${all(list, expression)}. Example: ${all(user.groups, is_allowed(data, current, 'read'))}"));
	}
