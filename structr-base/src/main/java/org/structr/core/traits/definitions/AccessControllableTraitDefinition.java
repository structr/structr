...
public final class AccessControllableTraitDefinition extends AbstractNodeTraitDefinition {
		...
		return Map.of(
			...
			new GetOwnerNode() {
				...
				public Principal getOwnerNode(final NodeInterface nodeInterface) {
					...
					final RelationshipInterface ownership = nodeInterface.getIncomingRelationshipAsSuperUser(StructrTraits.PRINCIPAL_OWNS_NODE);
					if (ownership != null) {
				...
				public void grant(final NodeInterface node, final Set<Permission> permissions, final Principal principal, final SecurityContext ctx) throws FrameworkException {
					...
					if (secRel == null) {
						...
						try {
							...
							final Traits traits                    = Traits.of(StructrTraits.SECURITY);
			...
			new GetSecurityRelationships() {
				...
				public List<Security> getSecurityRelationships(final NodeInterface node) {
					...
					final List<RelationshipInterface> grants = Iterables.toList(node.getIncomingRelationshipsAsSuperUser(StructrTraits.SECURITY, null));
	...
	private static boolean isGranted(final NodeInterface node, final Permission permission, final Principal accessingUser, final PermissionResolutionMask mask, final int level, final AlreadyTraversed alreadyTraversed, final boolean resolvePermissions, final boolean doLog, final Map<String, Security> incomingSecurityRelationships, final boolean isCreation) {
		...
		if (accessingUser != null) {
			...
			final Map<String, Security> localIncomingSecurityRelationships = incomingSecurityRelationships != null ? incomingSecurityRelationships : mapSecurityRelationshipsMapped(node.getIncomingRelationshipsAsSuperUser(StructrTraits.SECURITY, null));
