...
public class TableOfContentsMarkdownFileFormatter extends Formatter {
	...
	public boolean format(final List<String> lines, final Link link, final OutputSettings settings, final int level, final Set<Concept> seenConcepts) {
		...
		for (final Link childLink : markdownFile.getChildLinks(Verb.Has)) {
			...
			final Concept concept                 = childLink.getTarget();
			...
			for (final Map.Entry<Verb, List<Link>> child : concept.getChildLinks().entrySet()) {
				...
				if (Verb.Has.equals(verb)) {
					...
					for (final Link childChildLink : child.getValue()) {

						final Concept childConcept         = childChildLink.getTarget();
