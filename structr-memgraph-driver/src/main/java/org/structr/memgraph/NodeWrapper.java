...
class NodeWrapper extends EntityWrapper<org.neo4j.driver.v1.types.Node> implements Node {
	...
	public Iterable<Relationship> getRelationships(final Direction direction, final RelationshipType relationshipType, final String otherType) {
		...
		final String typeLabel         = otherType != null ? (":" + otherType) : "";

		switch (direction) {

			case BOTH:
				return cache.getResult(db, id, concat("(n", tenantIdentifier, ")-[r:", rel, "]-()"), concat("(n)-[:", rel, "]-(", typeLabel, ")"), "RETURN r ORDER BY r.internalTimestamp");
	...
	public static NodeWrapper newInstance(final MemgraphDatabaseService db, final long id) {
	...
	private static String getTenantIdentifer(final MemgraphDatabaseService db) {
	...
	private class RelationshipResult {
		...
		public synchronized Iterable<Relationship> getResult(final MemgraphDatabaseService db, final long id, final String match, final String pattern, final String returnStatement) {
