 ...
 *
 */
public class StructrCronTest {

	private static final Logger logger = LoggerFactory.getLogger(StructrCronTest.class.getName());
	...
	@Test
	public void testCronService() {

		try (final Tx tx = app.tx()) {

			app.create(SchemaMethod.class,
				new NodeAttribute<>(SchemaMethod.name, "test01"),
				new NodeAttribute<>(SchemaMethod.source, "log('test01')")
			);

			app.create(SchemaMethod.class,
				new NodeAttribute<>(SchemaMethod.name, "test02"),
				new NodeAttribute<>(SchemaMethod.source, "log('test02')")
			);

			app.create(SchemaMethod.class,
				new NodeAttribute<>(SchemaMethod.name, "test03"),
				new NodeAttribute<>(SchemaMethod.source, "log('test03')")
			);
	...
	public static void startSystem(final Map<String, Object> additionalConfig) {
		...
		config.setProperty(Services.CONFIGURED_SERVICES, "NodeService LogService SchemaService CronService");
		...
		config.setProperty("CronService.tasks", "test01 test02 test03");
		config.setProperty("test01.cronExpression", "0 0 9,17 * * 1-5");
		config.setProperty("test02.cronExpression", "* * * * * *");
		config.setProperty("test03.cronExpression", "1,3,10 * * * * *");
