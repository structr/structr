...
//			assertEquals("Invalid if(eq()) result with null value", "false",  Scripting.replaceVariables(ctx, testOne, "${eq(this.alwaysNull, 2.34462e-3)}"));
			...
			assertEquals("Invalid parse_date() result", "2015-12-12T00:00:00+0100", Scripting.replaceVariables(ctx, testOne, "${parse_date('2015-12-12', 'yyyy-MM-dd')}"));

			// to_date
			assertEquals("Invalid to_date() result", ToDateFunction.ERROR_MESSAGE_TO_DATE, Scripting.replaceVariables(ctx, testOne, "${to_date()}"));
			assertEquals("Invalid to_date() result", "2016-09-07T00:44:45+0200", Scripting.replaceVariables(ctx, testOne, "${to_date(1473201885000)}"));
