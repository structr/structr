...
var Resources = {	
    ...
    onload : function() {
        ...
        main.append('<table id="resourcesEditor"><tr><td id="previews"></td><td id="resources"></td><td id="elements"></td><td id="contents"></td></tr></table>');
    ...
    appendResourceElement : function(resource, resourceId) {
        ...
        $('.expand_icon', div).on('click', function() {
            Resources.toggleResource(this, resource);
        });
                ...
                if (structr_id) {
                    ...
                    el.on({
                        ...
                        blur: function() {
                            ...
                            self.removeClass('structr-editable-area-active');
                            Resources.updateContent(structr_id, self.text());
                        //Resources.reloadPreviews();
                        }
                    });
                    el.droppable({
                        accept: '.resource',
                        hoverClass: 'structr-editable-area-active',
                        drop: function(event, ui) {
                            console.log(event,ui);
                            var resourceId = getIdFromClassString(ui.draggable.attr('class'));
                            var targetId = $(this).attr('structr_id');
                            ...
                            if (sel.rangeCount) {
                                ...
                                Resources.linkResourcesToSelection(targetId, resourceId, selStart, selEnd);
    ...
    addElementToResource : function(elementId, resourceId) {
        ...
        element.append('<img title="Remove element ' + elementId + ' from resource ' + resourceId + '" '
            + 'alt="Remove element ' + elementId + ' from resource ' + resourceId + '" class="delete_icon button" src="' + Elements.delete_icon + '">');
    ...
    linkResourcesToSelection : function(targetId, resourceId, startOffset, endOffset) {
        if (debug) console.log('linkResourcesToSelection(' + targetId + ', ' + resourceId + ', ' + startOffset + ', ' + endOffset + ')');
        var data = '{ "command" : "LINK" , "id" : "' + targetId + '" , "data" : { "id" : "' + resourceId + '", "startOffset" : ' + startOffset + ', "endOffset" : ' + endOffset + '} }';
        return send(data);
    }
