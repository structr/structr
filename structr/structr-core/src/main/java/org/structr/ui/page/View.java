	...
	public void onRender() {
		...
		if (nodeToRender == null) {
			...
			if (editNodeId != null) {
				...
				editUrl = getContext().getRequest().getContextPath().concat(
					getContext().getPagePath(getEditPageClass(editNode))).concat("?").concat(
					NODE_ID_KEY).concat("=").concat(editNodeId.toString()).concat("&").concat(
					RENDER_MODE_KEY).concat("=").concat(INLINE_MODE);
			}
