...
public class CronEntry implements Delayed {
	...
	public long getDelayToNextExecutionInMillis() {
		...
		if(diffDow < 0)		{ diffDow     += 7; }
		if(diffMonths < 0)	{ diffMonths  += 30; }

		long next = 0;

		if(!seconds.isInside(nowSeconds))	next += (diffSeconds)	* (1000L);
		if(!minutes.isInside(nowMinutes))	next += (diffMinutes)	* (1000L * 60L);
		if(!hours.isInside(nowHours))		next += (diffHours)	* (1000L * 60L * 60L);
		if(!days.isInside(nowDays))		next += (diffDays)	* (1000L * 60L * 60L * 24L);
		if(!dow.isInside(nowDow))		next += (diffDow)	* (1000L * 60L * 60L * 24L *  7L);
		if(!months.isInside(nowMonths))		next += (diffMonths)	* (1000L * 60L * 60L * 24L * 30L);
	...
	public int compareTo(Delayed o) {
		...
		Long myDelay = new Long(getDelay(TimeUnit.MILLISECONDS));
		Long oDelay = new Long(o.getDelay(TimeUnit.MILLISECONDS));
