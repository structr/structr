...
//        List<StructrRelationship> rels = (List<StructrRelationship>) relsCommand.execute(this, relType, Direction.OUTGOING);
	...
	public User getOwnerNode()
	{
		for(StructrRelationship s : getRelationships(RelType.OWNS, Direction.INCOMING))
		{
			...
			logger.log(Level.SEVERE, "Owner node is not a user: {0}[{1}]", new Object[]
				{
					n.getName(), n.getId()
				});
		}
	...
	public void replaceBySubnodes(StringBuilder content, final AbstractNode startNode, final String editUrl, final Long editNodeId)
	{
		...
		while(start > -1)
		{
			...
			if(end < 0)
			{
				logger.log(Level.WARNING, "Node key suffix {0} not found in template {1}", new Object[]
					{
						NODE_KEY_SUFFIX, template.getName()
					});
				break;
			}
				...
				} else if(result instanceof AbstractNode)
				{
					...
					if(StringUtils.isNotEmpty(methodKey))
					{
						...
						try
						{
							...
							{
								logger.log(Level.FINE, "Cannot invoke method {0} on {1}", new Object[]
									{
										getter, s
									});
							}
...
//        }
//    }
	public void replaceByFreeMarker(final String templateString, Writer out, final AbstractNode startNode, final String editUrl, final Long editNodeId)
	{
	...
	public static void staticReplaceByFreeMarker(final String templateString, Writer out, final AbstractNode node, final String editUrl, final Long editNodeId)
	{
