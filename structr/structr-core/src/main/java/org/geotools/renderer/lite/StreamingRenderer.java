 ...
 * @version $Id: StreamingRenderer.java 36350 2010-11-21 10:34:06Z aaime $
 */
    ...
    public void paint(Graphics2D graphics, Rectangle paintArea,
        ...
        if (error > 0) {
            LOGGER.warning(new StringBuffer(
                    "Number of Errors during paint(Graphics2D, AffineTransform) = ").append(error).toString());
        }
...
//            }
    ...
    private MathTransform2D buildFullTransform(CoordinateReferenceSystem sourceCRS,
        ...
        if (mt != null && !mt.isIdentity()) {
            mt = (MathTransform2D) ConcatenatedTransform.create(mt, ProjectiveTransform.create(worldToScreenTransform));
        } else {
            ...
            if (attType == null) {
                if (LOGGER.isLoggable(Level.FINE)) {
                    LOGGER.fine(new StringBuffer("Could not find '").append(
                            attributes[j]).append("' in the FeatureType (").append(schema.getName()).append(")").toString());
                }
...
//                            java2dHints);
        ...
        if (featureSource != null) {
            ...
            if (LOGGER.isLoggable(Level.FINE)) {
                LOGGER.fine(new StringBuffer("processing ").append(
                        ...
                        currLayer.getFeatureSource().getSchema().getName()).toString());
            }
        ...
        for (Symbolizer symbolizer : symbolizers) {
            ...
            if (symbolizer instanceof RasterSymbolizer) {
                ...
                for (Property p : feature.getProperties()) {
                    ...
                    // don't put the complete geometry data into custom meta data!
    ...
    private boolean isOptimizedDataLoadingEnabled() {
        ...
        Object result = null;
        try {
            result = rendererHints.get("optimizedDataLoadingEnabled");
    ...
    private class RenderableFeature {
        ...
        public MetaDataShape getShape(Symbolizer symbolizer, AffineTransform at) throws FactoryException {
        ...
        private final MetaDataShape getTransformedShape(Geometry originalGeom, SymbolizerAssociation sa) throws TransformException,
                FactoryException {
    ...
    class PaintShapeRequest extends RenderingRequest {
        ...
        public PaintShapeRequest(Graphics2D graphic, MetaDataShape shape, Style2D style, double scale) {
    ...
    public class RenderRasterRequest extends RenderingRequest {
        ...
        void execute() {
            ...
            if (LOGGER.isLoggable(Level.FINE)) {
                LOGGER.fine(new StringBuffer("rendering Raster for feature ").append(content.toString()).append(" - ").append(
                        grid).toString());
            }
            ...
            try {
                ...
                if (grid instanceof GridCoverage) {
                    ...
                    if (params != null) {
                        ...
                        } else // we have no parameters hence we just use the read grid
