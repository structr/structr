	...
	onload = function() {
		...
		users.droppable({
			...
			drop: function(event, ui) {
				...
				var sourceId = getIdFromClassString(ui.draggable.attr('class'));
				//var targetId = getIdFromClassString($(this).attr('class'));
				//alert('dropped ' + sourceId + ' onto users');
				//removeSourceFromTarget(sourceId, targetId);
				//source.hide();
...
function appendGroupElement(group) {
	...
	div.droppable({
		...
		drop: function(event, ui) {
			...
			var sourceId = getIdFromClassString(ui.draggable.attr('class'));
			var targetId = getIdFromClassString($(this).attr('class'));
			//alert('dropped ' + sourceId + ' to ' + targetId);
			addSourceToTarget(sourceId, targetId);
			//source.hide();
			//source.appendTo($(this));
			$(this).append(source);
		}
