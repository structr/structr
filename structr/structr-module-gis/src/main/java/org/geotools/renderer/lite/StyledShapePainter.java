 ...
 *
 * @author Andrea Aime
 * @source $URL: https://svn.osgeo.org/geotools/tags/2.7-M4/modules/library/render/src/main/java/org/geotools/renderer/lite/StyledShapePainter.java $
 */
public final class StyledShapePainter {
     ...
     */
    public void paint(final Graphics2D graphics, final MetaDataShape shape,
     ...
     * @return
     */
    private PathIterator getPathIterator(final MetaDataShape shape) {
     ...
     * @param shape
     */
    void fillLiteShape(Graphics2D g, MetaDataShape shape) {
        if(shape.getGeometry() instanceof MultiPolygon && shape.getGeometry().getNumGeometries() > 1) {
            ...
            for (int i = 0; i < mp.getNumGeometries(); i++) {
                ...
                try {
                    g.fill(new MetaDataShape(p, null, null, false, false));
                } catch(Exception e) {
    ...
    private void paintGraphicFill(Graphics2D graphics, Shape shape, Style2D graphicFill, double scale)
    {
        ...
        for (int i = fromX; i < toX; i++)
        {
            for (int j = fromY; j < toY; j++)
            {
                ...
                // creates a MetaDataShape for the stipple and paints it
     ...
     * @return a MetaDataShape appropriate for painting a stipple using a GraphicFill.
     * @throws TransformException
     * @throws FactoryException
     */
    private MetaDataShape createStippleShape(Rectangle2D stippleSize, double translateX, double translateY)
    {
        ...
        // builds a MetaDataShape object from the JTS geometry
        AffineTransform2D identityTransf = new AffineTransform2D(new AffineTransform());
        Decimator nullDecimator = new Decimator(-1, -1);
        MetaDataShape stippleShape;
        try {
            stippleShape = new MetaDataShape(geom, identityTransf, nullDecimator, false);
        } catch(Exception e) {
