...
import {focusMonacoEditorInContainer} from "./helpers/pages";
...
test('user-defined-functions', async ({page}, testInfo) => {
	...
	await focusMonacoEditorInContainer(page, '#code-methods-container');
	...
	await expect(page.getByRole('button', {name: 'Save', exact: false})).toHaveAttribute('disabled');
	...
	await page.keyboard.type('$.sendPlain', { delay: 50 });
	...
	await expect(page.getByRole('button', {name: 'Save all'})).toHaveAttribute('disabled');
