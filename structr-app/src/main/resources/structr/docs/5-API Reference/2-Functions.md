...
|Name|Description|Optional|
|---|---|---|
|cause|cause of the error, e.g. a property name or other error message|no|
|errorToken|arbitrary string that represents the error, will end up in the `token` field of the response|no|
|detail|more detailed description of the error for humans to read|yes|
...
## sendPlaintextMail(from, fromName, to, toName, subject, content)
