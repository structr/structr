...
addEnvironmentEntryToConfig() {
		...
		local VALUE="${2}"
		local _STRUCTR_HOME="${3}"
		local CONF_FILE="${_STRUCTR_HOME}/structr.conf"

		# Create temp file, filter out existing setting and write back to original.
		# This is needed in case the file is mounted and can't be replaced.
		grep -v "^${SETTING}\s*=" "$CONF_FILE" > "${CONF_FILE}.tmp" && \
		cat "${CONF_FILE}.tmp" > "$CONF_FILE" && \
		rm -f "${CONF_FILE}.tmp"
