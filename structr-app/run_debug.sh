...
java -server -Djava.awt.headless=true -Djava.net.preferIPv4Stack=true -Djava.net.preferIPv6Addresses=false -Duser.timezone=$PROCESS_TZ -Duser.country=US -Duser.language=en -Djava.net.useSystemProxies=true -Dorg.apache.sshd.registerBouncyCastle=false -Xrunjdwp:transport=dt_socket,address=5005,server=y,suspend=$SUSPEND -Dfile.encoding=utf-8 $MEMORY_OPTS -XX:+UseG1GC -XX:+UseNUMA -XX:+UseCodeCacheFlushing --module-path ./target/lib -cp ./target/lib-classpath/*:./plugins/*:$LATEST --add-modules ALL-MODULE-PATH --enable-native-access=org.graalvm.truffle,io.netty.common --sun-misc-unsafe-memory-access=allow -m structr.base/org.structr.Server
