...
var _SimplePages = {
	...
	onload: async function() {
		...
		let html = await Structr.fetchHtmlTemplate('simple-pages/main', {});
	...
	showAddPageTemplateButton: async function(callback) {

		let html = await Structr.fetchHtmlTemplate('simple-pages/add-page-template-button', {});
	...
	listPages: function() {
		...
		Command.query('Page', 1000, 1, 'name', 'desc', { hidden: false }, async function(pages) {

			pages.forEach(async function(page) {

				let html = await Structr.fetchHtmlTemplate('simple-pages/page-list-node', { page: page });
			...
			let html = await Structr.fetchHtmlTemplate('simple-pages/create-page-button', {});
	...
	loadPage: function(pageId) {
		...
		Command.get(pageId, null, async function(page) {

			let pageRootHtml = await Structr.fetchHtmlTemplate('simple-pages/page-root', { page: page });
					...
					*/
					...
					doc.querySelectorAll('[data-structr-insert]').forEach(function(templateElement) {
						...
						if (!_SimplePages.objectCache.hasOwnProperty(id)) {
							...
							Command.get(id, null, async function(t) {

								_SimplePages.objectCache[id] = t;

								let templateEditHtml = await Structr.fetchHtmlTemplate('simple-pages/template-edit-elements', { template: t });

								templateElement.insertAdjacentHTML('afterBegin', templateEditHtml);

								//alert(templateEditHtml);
								//$(templateElement).append(templateEditHtml);
...
//								allTemplates
						...
						if (!_SimplePages.objectCache.hasOwnProperty(id)) {
							...
							Command.get(id, null, async function(t) {
								...
								let html = await Structr.fetchHtmlTemplate('simple-pages/widget-edit-elements', { widget: t });
	...
	appendTemplate: async function(template, parent, callback) {

		let html = await Structr.fetchHtmlTemplate('simple-pages/template-selection-node', { template: template });
	...
	listWidgets: function() {
		...
		Command.query('Widget', 1000, 1, 'name', 'desc', { hidden: false }, function(widgets) {
			...
			widgets.forEach(async function(widget) {
				...
				let html = await Structr.fetchHtmlTemplate('simple-pages/widget-list-node', { widget: widget });
