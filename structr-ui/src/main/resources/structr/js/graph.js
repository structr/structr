...
//        });
    ...
    appendObj: function(obj) {
        _Graph.loadTypeDefinition(obj.type, function(typeDef) {
            if (typeDef && typeDef.isRel) {
                relTypes.push(typeDef);

                if (obj.sourceId && obj.targetId) {
                    _Graph.loadNode(obj.sourceId, function() {
                        _Graph.loadNode(obj.targetId, function() {
            ...
            if (nodeIds.indexOf(node.id) === -1) {
                ...
                if (mode === 'auto') {
                    window.setTimeout(function() {
                        ...
                        _Graph.loadRelationships(node.id, depth === undefined ? 1 : depth + 1);
                ...
                success: function(data) {
                    ...
                    while (i < results.length && count < maxRels) {
                        ...
                        if (relIds.indexOf(r.id) === -1) {
                            ...
                            _Graph.loadNode(r.targetId, depth === undefined ? 1 : depth + 1);
                        }
                    }
                }
            });
                ...
                success: function(data) {
                    ...
                    while (i < results.length && count < maxRels) {
                        ...
                        if (relIds[r.id] === undefined) {
                            ...
                            engine.addRelationship(r.relType, r.sourceId, r.targetId);
                            _Graph.loadNode(r.sourceId, depth === undefined ? 1 : depth + 1);
                        }
                    }
                }
            });
    ...
    loadNode: function(nodeId, depth, callback) {
        if (nodeId) {
            Command.get(nodeId, function(n) {
                _Graph.appendNode(n, depth, callback);
            });
        }
    },
