...
public class UiScriptingTest extends StructrUiTest {
	...
	public void testSpecialHeaderPropertiesWithUnallowedProperty() {
		...
		try (final Tx tx = app.tx()) {
			...
			createTestNode(StructrTraits.RESOURCE_ACCESS,
					new NodeAttribute<>(Traits.of(StructrTraits.RESOURCE_ACCESS).key("signature"), "Folder/_Someprops"),
					new NodeAttribute<>(Traits.of(StructrTraits.RESOURCE_ACCESS).key("flags"), 1L),
	...
	public void testFunctionQueryWithJavaScriptAndRepeater() {
		...
		try (final Tx tx = app.tx()) {
			...
			// setup repeater
			content.setProperty(Traits.of(StructrTraits.DOM_NODE).key("functionQuery"), "{ var arr = []; for (var i=0; i<10; i++) { arr.push({ name: 'test' + i }); }; arr; }");
	...
	public void testExplicitOutputFunctionsInStrictActionContext() {
		...
		try (final Tx tx = app.tx()) {
			...
			app.create(StructrTraits.SCHEMA_METHOD, new NodeAttribute<>(Traits.of(StructrTraits.SCHEMA_METHOD).key("name"), "testPrintReturnJS"),
				new NodeAttribute<>(Traits.of(StructrTraits.SCHEMA_METHOD).key("source"), "{ $.print('testPrint'); 'returnValue'; }")
			);
			...
			app.create(StructrTraits.SCHEMA_METHOD, new NodeAttribute<>(Traits.of(StructrTraits.SCHEMA_METHOD).key("name"), "testPrintReturnUnreachablePrintJS"),
				new NodeAttribute<>(Traits.of(StructrTraits.SCHEMA_METHOD).key("source"), "{ $.print('testPrint'); 'returnValue'; $.print('unreachable print'); }")
			);
			...
			app.create(StructrTraits.SCHEMA_METHOD, new NodeAttribute<>(Traits.of(StructrTraits.SCHEMA_METHOD).key("name"), "testIncludeJS"),
				new NodeAttribute<>(Traits.of(StructrTraits.SCHEMA_METHOD).key("source"), "{ let val = $.include('namedDOMNode'); val; }")
			);
			...
			app.create(StructrTraits.SCHEMA_METHOD, new NodeAttribute<>(Traits.of(StructrTraits.SCHEMA_METHOD).key("name"), "testIncludeChildJS"),
				new NodeAttribute<>(Traits.of(StructrTraits.SCHEMA_METHOD).key("source"), "{ let val = $.include_child('namedDOMNode'); val; }")
			);

			app.create(StructrTraits.SCHEMA_METHOD, new NodeAttribute<>(Traits.of(StructrTraits.SCHEMA_METHOD).key("name"), "testRenderJS"),
				new NodeAttribute<>(Traits.of(StructrTraits.SCHEMA_METHOD).key("source"), "{ let val = $.render($.find('DOMNode', 'name', 'namedDOMNode')); val; }")
			);
