...
casper.test.begin(testName, numberOfTests, function(test) {
	...
	casper.then(function() {
		test.assertEvalEquals(function() {
			return ($('#pages .node.page .html_element > div.node:eq(1) > div.node:eq(1) > div:eq(0) .content_').text());
		}, result);

		// it seems this selector is too complicated for assertSelectorHasText
//		test.assertSelectorHasText('#pages .node.page .html_element > div.node:eq(1) > div.node:eq(1) > div:eq(0) .content_', result);
	});
